...
import {GetChain, GetPrecision} from '../../../../store/wallet/utils/currency';
import {
...
const BuyCryptoOffers: React.FC = () => {
  ...
  const setPrefix = (
    ...
    const prefix =
      ...
      BitpaySupportedEthereumTokens[_coin].paymentInfo.protocolPrefix[
        network
      ] ||
  ...
  const continueToSimplex = async () => {
    ...
    simplexPaymentRequest(selectedWallet, address, quoteData, createdOn)
      .then(req => {
        ...
        const destinationChain = dispatch(
          GetChain(selectedWallet.currencyAbbreviation),
        );
