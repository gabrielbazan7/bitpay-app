...
const signInput = async (params: {
  ...
  const signature = await new Promise<string>(
    async (resolveSign, rejectSign) => {
      ...
      // Detect a stalled session — either a restored session where the other
      // copayer never posted their round messages, or a live session that
      // made it through some rounds and then went silent (the other copayer
      // closed the app / lost connection mid-signing). Both surface the same
      // way: subscribe() polls forever and 'roundready' stops advancing.
      // We can't call start() with a fresh key share to recover: BWS already
      // has this session at a later round, so a new initJoin/round message
      // would collide and BWS would reject it with TSS_ROUND_ALREADY_DONE.
      // The only safe options are to pick up a signature that's already
      // complete on the server, or ask the user to retry with a new session.
      ...
      const handleStall = async () => {
        ...
        stalled = true;
        logManager.warn(
          `[TSS Sign] Input ${inputIndex + 1} Stall timeout after ${STALL_TIMEOUT_MS / 1000}s — no round progress`,
        );
        tssSign.unsubscribe();
        try {
          const sig = await tssSign.getSignatureFromServer();
          if (sig) {
            logManager.debug(
              `[TSS Sign] Input ${inputIndex + 1} recovered from server after stall timeout`,
            );
            await clearSigningSession(sessionId);
            resolveSign(toBwsSignatureFormat(sig, txp.chain));
            return;
      ...
      // Arm immediately for restored sessions: there may be no further
      // 'roundready' event if the other copayer's messages never arrive.
      if (savedSession) {
        armStallTimer();
      }
      ...
      tssSign
        .on('roundready', (round: number) => {
          // Round progress happened — reset the stall window instead of
          // cancelling it outright, so a deadlock that appears *after*
          // some progress (e.g. stuck at round 3) is still caught.
          armStallTimer();
          logManager.debug(
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        try {
          ...
          const errCode =
            (error as any)?.code ?? getErrorName(error as any);
          ...
          logManager.debug(
            `[TSS Sign] pushSignatures errCode: ${errCode} — ignorable: ${RACE_CONDITION_CODES.has(errCode)}`,
          );
